    ...
    <section className="py-16 px-6 bg-gray-100 flex justify-center">
      <div className="max-w-4xl w-full">
        ...
        <Carousel>
          ...
          <CarouselPrevious onClick={() => setCurrentIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length)} />
          <CarouselNext onClick={() => setCurrentIndex((prev) => (prev + 1) % testimonials.length)} />
        </Carousel>
      </div>
    </section>
  );
